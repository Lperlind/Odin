...
ApplicationDelegate :: struct {
	...
	terminate:                            proc "c" (self: ^ApplicationDelegate, sender: ^Object),
...
Application_setDelegate :: proc(self: ^Application, delegate: ^ApplicationDelegate) {
	...
	terminate :: proc "c" (self: ^Value, _: SEL, sender: ^Object) {
		del := (^ApplicationDelegate)(self->pointerValue())
		del->terminate(sender)
	}
	...
	class_addMethod(intrinsics.objc_find_class("NSValue"), intrinsics.objc_find_selector("terminate:"),                                       auto_cast terminate, "v@:@")
